...
public final class Database {
        ...
        try {
            ...
            try (Connection connection = getConnection(); Statement statement = connection.createStatement()) {
                ...
                statement.execute("CREATE TABLE IF NOT EXISTS appointments ("
                        ...
                        + ")");
                statement.execute("CREATE TABLE IF NOT EXISTS app_config ("
    ...
    private static void seed(Statement statement) throws SQLException {
        statement.executeUpdate("MERGE INTO users (username, password, full_name, role) KEY(username) "
                + "VALUES ('enfermera', '1234', 'Lic. Enfermeria', 'ENFERMERIA')");
        statement.executeUpdate("MERGE INTO users (username, password, full_name, role) KEY(username) "
                + "VALUES ('drlopez', '1234', 'Dr. Lopez', 'MEDICO')");
        statement.executeUpdate("MERGE INTO app_config (config_key, config_value) KEY(config_key) "
